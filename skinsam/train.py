...
from model import SkinSAM
from utils import AverageMeter, calculate_iou

torch.set_float32_matmul_precision("high")


def validate(fabric: L.Fabric, model: SkinSAM, val_dataloader: DataLoader, epoch: int = 0):
    model.eval()
    ious = AverageMeter()
...
def configure_optimizer(cfg: Box, model: SkinSAM):
    def lr_lambda(step):
...
def main(cfg: Box) -> None:
    fabric = L.Fabric(accelerator="auto",
                      ...
                      strategy="auto",
                      loggers=[TensorBoardLogger(cfg.out_dir, name="skin-sam")])
    fabric.launch()
    ...
    optimizer, scheduler = configure_optimizer(cfg, model)
    model, optimizer = fabric.setup(model, optimizer)

    train_skinsam(cfg, fabric, model, optimizer, scheduler, train_data, val_data)
